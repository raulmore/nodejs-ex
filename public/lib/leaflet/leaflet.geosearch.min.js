...
/*GeoSearch.HEREProvider = class HEREProvider extends GeoSearch.Provider {
	endpoint() {
		...
		return protocol + '//geocoder.cit.api.here.com/6.2/geocode.json?' + paramString;
	}
	parse(_ref2) {
		var data = _ref2.data;
		return data.Response.View && data.Response.View[0] ? data.Response.View[0].Result.map((r) => {
			return {
				x: r.Location.DisplayPosition.Longitude,
				y: r.Location.DisplayPosition.Latitude,
				label: r.Location.Address.Label,
				bounds: [[r.Location.MapView.BottomRight.Latitude, r.Location.MapView.TopLeft.Longitude], [r.Location.MapView.TopLeft.Latitude, r.Location.MapView.BottomRight.Longitude]],
				raw: r
			};
		}) : [];
	}
