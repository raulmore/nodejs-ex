...
(function() {
window.createTree = function (p_div,p_backColor,p_contextMenu) {
	var tree = {
		name: 'tree',
		div: p_div,
		ulElement: null,
		...
		createNode: function(p_text,p_expanded, p_icon, p_parentNode,p_tag,p_contextmenu) {
			...
			node = {
				...
				setIcon: function(p_icon) { v_tree.setIcon(this,p_icon); },
				///// Collapsing the node
			...
			if (this.rendered) {
				if (p_parentNode==undefined) {
					...
					if (p_parentNode.childNodes.length==0) {
						...
						v_img.src = 'lib/aimaraJS/images/collapse.png';
						v_img.id = 'toggle_off';
						}
						else {
							...
							v_img = p_parentNode.elementLi.getElementsByTagName("img")[0];
							v_img.style.visibility = "visible";
							v_img.src = 'lib/aimaraJS/images/expand.png';
							v_img.id = 'toggle_on';
						}
					}
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.icon!=null)
				v_icon = createIElement(null,p_node.icon);
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement('toggle_off','exp_col','lib/aimaraJS/images/collapse.png');
				...
				if (p_node.expanded) {
					...
					v_exp_col = createImgElement('toggle_on','exp_col','lib/aimaraJS/images/expand.png');
				}
			}
			...
			//v_span.appendChild(createImgElement(null, 'icon_tree', ''));
		...
		setIcon: function(p_node,p_icon) {
			p_node.elementLi.getElementsByTagName('span')[0].firstChild.className = p_icon;
			p_node.icon = p_icon;
		},
		...
		expandNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				img.id="toggle_off";
				img.src = 'lib/aimaraJS/images/collapse.png';
		...
		collapseNode: function(p_node) {
			if (p_node.childNodes.length>0 && p_node.expanded==true) {
				...
				img.id="toggle_on";
				img.src = 'lib/aimaraJS/images/expand.png';
				elem_ul = img.parentElement.getElementsByTagName("ul")[0];
				...
				if (p_node.contextMenu!=undefined) {
					...
					for (var i=0; i<v_menu.elements.length; i++) (function(i){
						...
						if (v_menu.elements[i].submenu!=undefined) {
							...
							v_span_more.appendChild(createImgElement(null,'menu_img','lib/aimaraJS/images/right.png'));
...
function createIElement(p_id,p_class) {
